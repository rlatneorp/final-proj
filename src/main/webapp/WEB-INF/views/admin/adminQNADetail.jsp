			...
			<form action="${contextPath}/adminQNAUpdate.ad" method="post" id="updateForm">
				...
				<div class="row" style="padding: 0px 35px 0px 35px;">
					...
					<input type="text" class="col-2 mb-3" 
						value='<c:if test="${qna.qnaType eq 1}">배송</c:if><c:if test="${qna.qnaType eq 2}">결제</c:if><c:if test="${qna.qnaType eq 3}">회원</c:if><c:if test="${qna.qnaType eq 0}">기타</c:if>' readonly>
